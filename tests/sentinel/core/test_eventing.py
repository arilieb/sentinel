...
class TestSyncServerKeyState(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_sync_server_key_state_fetch_receipts_from_witnesses(
        ...
        mock_hab.fetchUrls.return_value = {
            "http": "http://witness.example.com"
        }
        ...
        # Setup urljoin
        mock_urljoin.return_value = "http://witness.example.com/receipts?pre=ETestPrefix123&sn=1"
    ...
    async def test_sync_server_key_state_fetch_from_delegator_success(
        ...
        mock_db.getKelIter.return_value = [b'digest1']
        mock_db.getEvt.return_value = b'event_raw_data'
        mock_db.fetchLastSealingEventByEventSeal.return_value = Mock(sn=5, saidb=b'delegator_said')
        mock_hab.db = mock_db
        ...
        mock_multipart_decoder_class.from_response.assert_called_once_with(mock_response)
    ...
    async def test_sync_server_key_state_delegator_sequence_mismatch(
        ...
        doc_data = {
            'key_state': {
                's': 'a'  # hex for 10, different from kevers sn of 5
            }
        }
        doc_part.content = json.dumps(doc_data).encode('utf-8')
