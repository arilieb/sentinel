...
async def sync_watched_identifier(
    hby: Habery, essr: "APIClient", aid: str
) -> dict[str, Any]:
    """
    ...
    """
    try:
        response = await essr.request(path=f"/watched/{aid}", method="GET", timeout=30)
        ...
        if response and response.status_code in (200, 202):
            ...
            for part in decoder.parts:
                msg = Message()
                msg["content-type"] = part.headers[b"content-disposition"].decode(
                    "utf-8"
                )
