...
parser.add_argument("--name", "-n",
                    help="Name of the database environment", required=True)
...
parser.add_argument('-u', '--uxd', action="store_true",
                    help="Listen on Unix Domain Sockets interface instead of TCP")
parser.add_argument('--port', '-P', action='store', default=35776,
...
async def async_run_sentinel(args):
    ...
    # Setup services
    if args.local:
        services = sentineling.setup_local(name=args.name,
                                          alias=args.alias,
                                          base=args.base,
                                          bran=args.bran,
                                          uxd=args.uxd,
    ...
    for service in services:
        if hasattr(service, 'start'):
            ...
            logger.warning(f"Service {service.__class__.__name__} does not have a start() method")
